...
def _find_msfconsole() -> Optional[str]:
    """Return full path to msfconsole or None."""
    candidates = [
    ...
    try:
        result = subprocess.run(
            ["where", "msfconsole"] if sys.platform == "win32" else ["which", "msfconsole"],
    ...
    def _collect_until_prompt(self, timeout: float = None) -> List[str]:
        ...
        while time.time() < deadline:
            try:
                line = self._out_q.get(timeout=1)
                lines.append(line.rstrip())
                if ("msf" in line and ">" in line) or line.strip().endswith("msf6 >"):
                    break
            except queue.Empty:
                continue
        return lines
